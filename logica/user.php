...
class User{
    ...
    public function RegistrarUsuario($nombre, $edad, $localidad, $idiomas, $etiquetas, $niveles_idiomas, $niveles_etiquetas, $email, $clave, $descripcion, $estado, $fecha_act, $fotoPerfil, $portfolio, $tipo, $habilitado, $url){
        ...
        try{
            if($comprobante == false){
                $stmt = $connection->prepare("INSERT INTO `usuarios`(`usuario_nombre`, `usuario_edad`, `usuario_localidad`, `usuario_email`, `usuario_clave`, `usuario_descripcion`, `usuario_dateAct`, `usuario_fotoPerfil`, `usuario_portfolio`, `usuario_habilitado`, `usuario_tipo`, `usuario_estado`) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
...
if(isset($_POST['action']) && $_POST['action'] == '1'){
    ...
    $obj_user->RegistrarUsuario($name, $age, $location, $array_languages, $array_labels, $array_languages_levels, $array_labels_levels, $mail, $password, $description, $state, $date, $picture, $briefcase, $type_user, $enabled, $url_recibida);
